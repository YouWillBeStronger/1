        ...
        while (true) {
            ...
            switch (command) {
                case 1:
                    ...
                    break;
                case 2:
                    //用户账户开户(alt + enter 快捷键
                    register(accounts, sc);
                    break;
                default:
                    System.out.println("您输入的操作命令 不存在~~");
            }
        }
    }
    ...
    private static boolean deleteAccount(Account acc, Scanner sc, ArrayList<Account> accounts) {
        ...
        switch (rs){
            case "y":
                //真正销户
                if(acc.getMoney() > 0){
                    System.out.println("您账户中还有钱没取完，不能销户");
                }else {
                    accounts.remove(acc);
            ...
            if(acc.getPassWord().equals(passWord)){
                while (true) {
                    ...
                    if(newPassWord.equals(okPassWord)){
                        ...
                        System.out.println("您输入2次的密码不一致");
                    }
                }
    ...
    private static void drawMoney(Account acc, Scanner sc) {
        ...
        while (true) {
            ...
            if (money > acc.getQuotaMoney()) {
                System.out.println("对不起，您当前取款的金额超过每次限额，每次最多可取：" + acc.getQuotaMoney());
            } else {
                //没有超过档次限额
                //4.判断是否超过了账户的总余额
                if(money > acc.getMoney()){
        ...
        private static void register (ArrayList < Account > accounts, Scanner sc){
            ...
            while (true) {
                ...
                if (okPassWord.equals(passWord)) {
                    ...
                    System.out.println("对不起，您输入两次密码不一致，请重新认证");
                }
            }
        ...
        private static String getRandomCardId (ArrayList < Account > accounts) {
            while (true) {
                ...
                Account acc = getAccountByCardId(cardId, accounts);
                if (acc == null) {
                    //说明cardId此时没有重复，这个考好是一个新卡号，可以使用这个卡号作为新注册的卡号了
                    return cardId;
                }
            }
        }
